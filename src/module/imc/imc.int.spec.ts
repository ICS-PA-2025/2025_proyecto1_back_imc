...
(shouldRunIntegrationTests ? describe : describe.skip)(
  'ImcService (integración, PostgreSQL real)',
  () => {
    ...
    beforeAll(async () => {
      ...
      try {
        ...
        console.log(
          'Error connecting to database for integration tests:',
          error.message,
        );
        throw error;
      }
    ...
    it('debería lanzar error si el peso es <= 0', async () => {
      ...
      await expect(service.calcularImc(dto, testUserId)).rejects.toThrow(
        BadRequestException,
      );
    ...
    it('findAll debería retornar el historial completo', async () => {
      ...
      await service.calcularImc({ altura: 1.8, peso: 80 }, testUserId);
